...
import { getWeather } from "@/lib/api"; // Server Action
...
export default function CuacaPage() {
  ...
  const [villages, setVillages] = useState<Area[]>([]); // Tambahan: Kelurahan/Desa

  const [selectedProv, setSelectedProv] = useState("");
  const [selectedReg, setSelectedReg] = useState("");
  const [selectedDist, setSelectedDist] = useState("");
  const [selectedVill, setSelectedVill] = useState(""); // Tambahan: Kelurahan/Desa
  ...
  // 1. Fetch Provinsi & Coba Auto-Detect GPS
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          ...
          if (cityName) {
            setStatusMsg(`Lokasi terdeteksi: ${cityName}. Memuat cuaca...`);
          }
          
          // Menggunakan lokasi default aman jika tidak ada pencocokan teks kelurahan eksak
          fetchWeatherAction("31.71.03.1001"); // Default: Gambir, Jakarta Pusat
          setStatusMsg("");
  ...
  const fetchWeatherAction = async (adm4Code: string) => {
    ...
    try {
      ...
      if (!data || !data.data || data.data.length === 0) {
        setWeatherError("Data Cuaca Tidak Tersedia. Stasiun BMKG belum menyediakan data prakiraan cuaca spesifik untuk wilayah ini.");
      } else {
        setWeatherData(data.data[0]);
      }
    } catch (err) {
      setWeatherError("Terjadi kesalahan saat menghubungkan ke server.");
    } finally {
      setWeatherLoading(false);
    }
  };

  // 6. Handle Tampilkan Cuaca
  const handleTampilkan = () => {
    if (!selectedVill) return;
    // Karena dropdown sudah mencapai level Kelurahan (adm4), langsung panggil API!
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-12 overflow-hidden">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="space-y-4 mb-10 bg-slate-900/50 p-6 rounded-2xl border border-slate-800">
          ...
          {/* Grid Layout 4 Kolom untuk 4 Level Wilayah */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {/* Provinsi */}
            <div className="flex flex-col gap-2">
              <label className="text-xs text-slate-400 uppercase font-bold tracking-wider">Provinsi</label>
              <select
                value={selectedProv}
                onChange={(e) => {
                ...
                className="bg-slate-800 text-white border border-slate-700 rounded-lg p-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none cursor-pointer"
              >
            ...
            {/* Kabupaten */}
            <div className="flex flex-col gap-2">
              <label className="text-xs text-slate-400 uppercase font-bold tracking-wider">Kota/Kabupaten</label>
              <select
                ...
                className="bg-slate-800 text-white border border-slate-700 rounded-lg p-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none disabled:opacity-50 cursor-pointer"
              >
            ...
            {/* Kecamatan */}
            <div className="flex flex-col gap-2">
              <label className="text-xs text-slate-400 uppercase font-bold tracking-wider">Kecamatan</label>
              <select
            ...
            {/* Kelurahan (BARU) */}
            <div className="flex flex-col gap-2">
              <label className="text-xs text-slate-400 uppercase font-bold tracking-wider">Kelurahan / Desa</label>
              <select
                disabled={!selectedDist || locLoading}
                value={selectedVill}
          ...
          <div className="mt-6 flex justify-end border-t border-slate-800 pt-4">
            ...
            >
              {locLoading || weatherLoading ? "Memproses..." : "Tampilkan Cuaca Kelurahan Ini"}
            </button>
          </div>
        </div>

        {/* --- KOMPONEN HASIL CUACA --- */}
        {weatherLoading ? (
          ...
          <>
            ...
            {/* Prakiraan Cuaca Lengkap Mendatang (BARU - Horizontal Scroll) */}
            <div className="mt-12 bg-slate-900/40 p-6 md:p-8 rounded-3xl border border-slate-800">
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-xs text-slate-400 bg-slate-800 px-3 py-1 rounded-full hidden md:inline-block">Geser untuk melihat lebih lanjut →</span>
              </div>
              
              {/* Container Horizontal Scroll */}
              <div className="flex overflow-x-auto pb-6 pt-2 gap-4 snap-x snap-mandatory hide-scrollbar" style={{ scrollbarWidth: 'thin', scrollbarColor: '#334155 transparent' }}>
                
                {/* Menampilkan SELURUH data array yang tersedia (tidak di-slice lagi) */}
                {weatherData.cuaca[0].map((forecast: any, idx: number) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs text-slate-400 mt-3 text-center capitalize font-medium leading-tight h-8 z-10">
                        {forecast.weather_desc}
                      </span>

                      {/* Detail ekstra kecil di bawah */}
                      <div className="mt-3 pt-3 border-t border-slate-700/50 w-full flex justify-between text-[10px] text-slate-500 font-mono z-10">
                        <span>💧 {forecast.hu}%</span>
                        <span>💨 {forecast.ws}k/j</span>
                      </div>
                    </div>
                  );
                })}
